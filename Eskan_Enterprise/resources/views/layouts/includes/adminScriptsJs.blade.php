    ...
    //----- Open model CREATE -----//
            ...
            $('thead').on('click', '.addRow', function () {
                var trForm =
                '@csrf<div class="row d-inline-flex">'+
                ...
                '<div class="col-lg-1">'+
                    ...
                    '<select class="custom-select l  font-weight-bold text-dark" style="width='20px'" name="construction_id" >'+
                        '<option selected>القسم</option>@foreach ( $properties as $property)'+
                        '<option value="{{ $property->id }}">{{ $property->name }}</option>@endforeach</select>'+
                '</div>'+

                '<div class="col-lg-1"><label for="">المشروع</label><select class="custom-select l  font-weight-bold text-dark" style="width='20px'" name="construction_id" ><option selected>المشروع</option> @foreach ( $main_projects as $main_project)<option value="{{ $main_project->id }}">{{ $main_project->name }}</option>@endforeach</select></div>'+
                ...
                '<div class="col-lg-1"><label for="">حالة الوحدة</label><select class="custom-select l  font-weight-bold text-dark" style="width='20px'" name="status" ><option selected value="خالية">خالية</option><option value="محجوزة">محجوزة</option><option value="تعاقد">تعاقد</option><option value="ملغاة">ملغاة</option></select></div>'+

                '<div class="col-lg-1"><label for="">العميل</label><select class="custom-select l  font-weight-bold text-dark" style="width='20px'" name="customer_id" ><option  selected value="0">عميل</option>@foreach ( $customers as $customer)<option value="{{ $customer->id }}">{{ $customer->name }}</option>@endforeach</select></div>'+

                '<div class="col-md-12 mb-3"><button type="submit" class="btn btn-primary">اضافة</button></div>'+
                '<div class="col-lg-1 addRow"><a href="">+</a></div>'+
                '</div>' ;
            })
        </script>
